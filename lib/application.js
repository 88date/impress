...
const { AsyncLocalStorage } = node.asyncHooks;
...
const { Pgboss, getPgbossConfig } = require('./pgboss.js');
...
class Application extends EventEmitter {
  constructor() {
    ...
    this.service = new Service('api', this);
    this.lib = new Code('lib', this, { typescript: true });
    this.db = new Code('db', this);
    this.bus = new Code('bus', this);
    this.tasks = new Tasks(this);
    this.mq = new MQ('mq', this);
    this.eventPublisher = null;
    this.subscriptions = null;
    this.events = null;
    this.subscribers = null;
    this.integration = new Code('integration', this, { typescript: true });
    this.domain = new Code('domain', this, { typescript: true });
    this.modules = new Code('modules', this, { typescript: true });
  ...
  initializeMessaging() {
    const { config, console } = this;
    const pgbossConfig = getPgbossConfig(
      config.server.pgboss,
      config.server.scheduler,
    );
    const pgboss = new Pgboss(pgbossConfig, console);
    let nats = null;
    if (config.server.nats.enabled) {
      const discovery = {
        loader: workerData.discoveryLoader === true,
        request: () => parentPort.postMessage({ name: 'catalogRequest' }),
        publish: (services) =>
          request(parentPort, { name: 'catalogUpdate', services }),
      };
      const eventDiscovery = {
        loader: workerData.discoveryLoader === true,
        request: () => parentPort.postMessage({ name: 'eventCatalogRequest' }),
        publish: (events) =>
          request(parentPort, { name: 'eventCatalogUpdate', events }),
      };
      nats = new Nats(this, discovery, eventDiscovery);
    }
    const eventPublisher = new EventPublisher(pgboss, nats);
    const subscriberNats = this.kind === 'server' ? nats : null;
    const managesTopology =
      config.server.pubsub?.active === true &&
      workerData.discoveryLoader === true;
    const local = new PgbossSubscriptions(pgboss, managesTopology);
    const natsSubscriptions = new NatsSubscriptions(subscriberNats, console);
    ...
    Object.assign(this, { pgboss, nats, eventPublisher, subscriptions });
    const onEventsChanged = () => nats?.updateEventDiscovery();
    this.events = new EventLoader(this, subscriptions, onEventsChanged);
    this.subscribers = new SubscriberLoader(this, subscriptions);
  }
  ...
  async start() {
    ...
    if (this.nats) await this.nats.start();
    ...
    if (kind === 'server' || kind === 'balancer') {
      ...
      sandbox.application.clients = this.server.clients;
    }
    if (sandbox.api.auth) {
  ...
  async shutdown() {
    ...
    if (this.subscriptions) await this.subscriptions.stop();
    if (this.eventPublisher) await this.eventPublisher.stop();
    if (this.nats) await this.nats.close();
    await this.pgboss?.stop(this.config?.server?.timeouts?.stop);
  ...
  createSandbox() {
    ...
    const getDocumentation = () => this.getDocumentation();
    const events = {
      emit: (...args) => this.eventPublisher.emit(...args),
    };
    const data = {
      worker,
      server,
    ...
    const sandbox = { ...SANDBOX, console, application, config, events };
    sandbox.api = {};
    sandbox.service = {};
    sandbox.lib = this.lib.tree;
    sandbox.db = this.db.tree;
    sandbox.bus = this.bus.tree;
    sandbox.tasks = this.tasks.tree;
    sandbox.mq = this.mq.tree;
    sandbox.mq.getStatus = () => this.mq.getStatus();
    sandbox.integration = this.integration.tree;
    sandbox.domain = this.domain.tree;
    sandbox.modules = this.modules.tree;
    sandbox.schemas = this.schemas.model;
    this.sandbox = metarhia.metavm.createContext(sandbox);
  }

  getMethod(name, ver, methodName, transport) {
    ...
    if (!proc || !proc.transports.includes(transport)) return null;
    return proc;
  }
  ...
  startWatch() {
    ...
    this.watcher.on('change', (filePath) => {
      ...
      node.fs.stat(filePath, (error, stat) => {
        ...
        if (filePath.includes('#')) {
          if (place === 'api') this.service.load(this.service.path);
          return void target.load(target.path);
        }
        if (stat.isDirectory()) {
          if (place === 'api') this.service.load(filePath);
          return void target.load(filePath);
        }
        if (threadId === 1) this.console.debug(`Reload: /${relPath}`);
        if (place === 'api') this.service.change(filePath);
    ...
    this.watcher.on('delete', async (filePath) => {
      ...
      const target = this[place];
      if (!target) return;
      if (place === 'api') this.service.delete(filePath);
  ...
  async getDocumentation() {
    const discovered = this.nats ? await this.nats.getCatalog() : null;
    const definitions = await metarhia.metaschema.readDirectory(
      this.schemas.path,
    );
    return {
      api: describeApi(this.api.collection),
      services: describeServices(this.service.collection, discovered),
      schemas: describeSchemas(definitions),
      queues: describeQueues(this.mq.configs, this.mq.handlers),
      events: describeEvents(
        this.subscriptions?.describeEvents({ natsOnly: false }),
        this.nats?.eventCatalog,
      ),
    };
  }
}
