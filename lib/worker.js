...
const handlers = {
  ...
  task: (message) => application.tasks.handle(message),

  catalog: ({ snapshot }) => application.nats?.applyCatalog(snapshot),

  eventCatalog: ({ snapshot }) => application.nats?.applyEventCatalog(snapshot),
