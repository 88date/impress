...
class Nats {
  ...
  subscribeDiscoveryChanges() {
    ...
    const callback = async (error, message) => {
      ...
      try {
        await this.discoverService(name);
      } catch (error) {
        this.application.console.error(error);
      }
    };
  ...
  subscribeDiscoveryCatalog() {
    if (this.discoveryCatalogSubscription) return;
    const callback = (error, message) => {
      if (error) {
        this.application.console.error(error);
        return;
  ...
  async discoverServices() {
    const { service } = this.application;
    const { maxWait } = this.application.config.service.discovery;
    const discovered = new Map();
    let responses = 0;
    try {
      const options = { strategy: 'timer', maxWait };
    ...
    if (responses === 0) throw new Error('Service discovery failed');

    const units = Object.keys(service.collection);
    const remoteNames = units.filter((name) => service.isRemote(name));
    for (const [name, actions] of discovered) {
      service.loadRemote(name, Array.from(actions.values()));
    }
    for (const name of remoteNames) {
      if (!discovered.has(name)) service.loadRemote(name, []);
    }
  }
  ...
  discoverService(name) {
    ...
    const request = this.requestDiscovery(name);
  ...
  async requestDiscovery(name) {
    ...
    try {
      ...
      if (error instanceof NoRespondersError) {
        this.application.service.loadRemote(name, []);
        return;
      }
    ...
    this.application.service.loadRemote(name, Array.from(actions.values()));
  }
