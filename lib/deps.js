...
const node = {};
const npm = { postgres, dayjs, stripe };
const metarhia = {};
...
const internals = [...builtinModules, 'node:test'].filter(isSupported);
...
const npmpkg = ['ws', 'postgres', 'dayjs'];
...
for (const name of dependencies) {
  ...
  if (name === 'stripe') continue;
  if (name === 'dayjs') continue;
  let lib = null;
  try {
