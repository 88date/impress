...
const NESTED_OPTIONS = ['queue', 'job', 'worker', 'schedule'];
...
const createDeclaration = (unit, taskPath) => {
  ...
  for (const field of NESTED_OPTIONS) {
    if (unit[field] !== undefined) {
      throw new Error(`Task "${taskPath}": ${field} must be flattened`);
    }
  }
  if (typeof unit.cron !== 'string') {
    throw new Error(`Task "${taskPath}": cron expected`);
  }
  if (unit.notify !== undefined) {
    throw new Error(`Task "${taskPath}": use server.scheduler.notify`);
  }

  const name = TASK_NAMESPACE + taskPath;
  const queue = selectOptions(unit, QUEUE_OPTIONS);
...
const collect = (level, parentPath, result) => {
  for (const [name, unit] of Object.entries(level)) {
    ...
    const isTask =
      unit.schedule !== undefined ||
      unit.cron !== undefined ||
      unit.method !== undefined;
    if (isTask) {
