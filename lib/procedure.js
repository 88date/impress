...
class Procedure {
  ...
  leave(ip) {
    ...
    if (this.exports.queue) {
      ...
      if (semaphore) {
        semaphore.leave();
        if (semaphore.empty) this.locks.delete(ip);
      }
    }
  }
  ...
  async invoke(context, args = {}) {
    ...
    if (validate) {
      let check = await validate(args);
      if (metarhia.metautil.isError(check)) {
        const isDomainError = check instanceof DomainError;
        if (isDomainError) check = check.toError(errors);
        return check;
      }
    }
    if (roles) {
    ...
    let promise = this.application.contextStorage.run(context, invoke);
