...
const { checkRoles, checkSegments } = require('./role.js');
...
class Procedure {
  ...
  async invoke(context, args = {}) {
    const { parameters, validate, returns, errors, timeout, roles, segments } =
      this;
    ...
    if (roles) {
      ...
      if (!allowed) return new Error('Forbidden', { code: 403 });
    }
    if (segments) {
      const allowed = checkSegments(context, segments);
      if (!allowed) return new Error('Forbidden', { code: 403 });
    }
