...
class Broker {
  ...
  get config() {
    return this.application.service.getConfig(this.serviceName);
  }
  ...
  call(args = {}) {
    ...
    return this.application.nats.request(this.subject, args, request.timeout);
  }
...
class EventBroker extends EventEmitter {
  ...
  describe() {
    const events = [];
    for (const [name, event] of Object.entries(this.collection)) {
      events.push({
        name,
        parameters: event.exports.parameters || null,
        caption: event.caption,
        description: event.description,
