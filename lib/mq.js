...
class MQ extends Place {
  ...
  async rebuildQueue(queueName) {
    ...
    const mqConfig = application.config.mq;
    ...
    if (application.kind === 'worker') {
      ...
      const processor = async (job) => {
        ...
        if (!handler) {
          const msg = `Unknown job "${job.name}"`;
          throw new Error(msg);
        }
  ...
  syncQueueEvents(queueName) {
    ...
    const isServer = application.kind === 'server';
    if (needsEvents && isServer && !this.queueEvents[queueName]) {
